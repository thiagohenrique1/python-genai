...
def create_update_get(client, parameters):
  ...
  updated_cache = client.caches.update(
      name=cache.name,
      config=genai_types.UpdateCachedContentConfig(ttl="3600s"),
  )
  got_cache = client.caches.get(name=updated_cache.name)
  client.caches.delete(name=got_cache.name)


test_table: list[pytest_helper.TestTableItem] = [
    pytest_helper.TestTableItem(
        name="test_create_update_get_vertex",
        ...
        exception_if_mldev="unsupported",
    ),
]
